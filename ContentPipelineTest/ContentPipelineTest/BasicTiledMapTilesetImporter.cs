...
{
    [ContentImporter(".tmx", DisplayName = "BasicTiledMapTilesetImporter", DefaultProcessor = "BasicTiledMapTilesetProcessor")]
    public class BasicTiledMapTilesetImporter : ContentImporter<BasicTiledMapTilesetContent>
    {
        public override BasicTiledMapTilesetContent Import(string filename, ContentImporterContext context)
        {
            context.Logger.LogMessage($"=== Starting TMX Import for {filename} ===");
            BasicTiledMapTilesetContent result = new()
            {
                Filename = Path.GetFullPath(filename),
                Tilesets = new Dictionary<string, BasicParallelTileset>(),
                Layers = new Dictionary<string, BasicParallelLayer>()
            };
        ...
        private BasicParallelTileset LoadBasicTileset(XmlReader reader, ContentImporterContext context)
        {
            ...
            BasicParallelTileset result = new();
                        ...
                        switch (name)
                        {
                            ...
                            case "tile":
                                ...
                                if (currentTileId != -1)
                                {
                                    ...
                                    if (!result.TileProperties.TryGetValue(currentTileId, out BasicParallelTileset.TilePropertyList props))
                                    {
                                        props = new BasicParallelTileset.TilePropertyList();
        ...
        private BasicParallelLayer LoadBasicLayer(XmlReader reader, string filename, ContentImporterContext context)
        {
            context.Logger.LogMessage("\n=== Starting Layer Loading ===");
            CultureInfo ci = (CultureInfo)CultureInfo.CurrentCulture.Clone();
            ci.NumberFormat.CurrencyDecimalSeparator = ".";
            BasicParallelLayer result = new();
            BasicParallelLayer.FlippedHorizontallyFlag = 0x80000000;
            BasicParallelLayer.FlippedVerticallyFlag = 0x40000000;
            BasicParallelLayer.FlippedDiagonallyFlag = 0x20000000;
            BasicParallelLayer.HorizontalFlipDrawFlag = 1;
            BasicParallelLayer.VerticalFlipDrawFlag = 2;
            BasicParallelLayer.DiagonallyFlipDrawFlag = 4;
